...
void Task_Infrared(void *p_arg)
{
    ...
    for(;;)
    {
        ...
        OSSemPend( CL_LY.SemCLCopyOver,0,&err);


        if(CL_Format.FramLenth == 9 && CL_Format.address == 0xAAAA)
        {
         
          /* suspend task self explicit task */
            switch(CT_Info.Datadirection)
            {
            case Wireless_Port:
              
                /* suspend wireless receive task */
                OSTaskSuspend(TASK_WIRELESS_PRIO);;
                break;
				
            case CAN_Port:
              
                /* suspend can receive task */
                OSTaskSuspend(TASK_CANRxCL_PRIO);
                break;
            ...
            case RS485_Port:
                ...
                /* suspend 485 port */
                RS485_PWR_DISABLE  //disable 12V DC OUT
                OSTaskSuspend(TASK_USART3CL_PRIO);
                break;
            ...
            case RS485_Port:
                RS485_Rx_EN
                RS485_PWR_ENABLE  //enable 12V DC OUT
        ...
        else
        {
            ...
            case RS485_Port:
                ...
                OSSemPend(CL_LY.Sem485TxComplete,100ul,&err); //wait for Tx complete
...
void Task_USART1_CL(void *p_arg)
{
    ...
    CL_LY.SemCLCopyOver = OSSemCreate(0);
...
void Task_USART3_CL(void *p_arg)
{
    ...
    for(;;)
    {
        ...
        while(USART1_DMASendData(CL_LY.RS485Rxdata,CL_LY.RS485RxLenth) == 0);
